...
public class ProductRetailler {
	...
	public String search(@RequestParam("id") int id,ModelMap map) {
		...
		Product bean = service.searcheProduct(id);
		if(bean==null) {
	...
	public String buy(@RequestParam("bid") int bid, ModelMap map) {
		Product bean = service.buyOneProduct(bid);
		if(bean == null) {
			map.addAttribute("msg", "Failed To Buy!!!");
		}else {
			
			map.addAttribute("msg", "One Product "+ bean.getProductName()+ " bought Successfully whose cost is: " +bean.getPriceperUnit() );
		}
		return "home";
	}
